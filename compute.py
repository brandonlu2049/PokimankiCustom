...
def FirstDeckPokemon() -> None:
    ...
    if ok and inp:
        ...
        if hasattr(msgbox, 'change_icon_path'): msgbox.display_image_below_text("Bulbasaur") # ｶｽﾀﾑ
        if hasattr(msgbox, 'change_icon_path'): msgbox.display_image_below_text("Charmander") # ｶｽﾀﾑ
        if hasattr(msgbox, 'change_icon_path'): msgbox.display_image_below_text("Squirtle") # ｶｽﾀﾑ
        msgbox.exec()
        clicked_button = msgbox.clickedButton()
        if clicked_button is not None:
            deckmon = clicked_button.text()
        else:
            deckmon = None
        if deckmon:
            deck = mw.col.decks.by_name(inp)["id"] #byNameを変更
...
def MultiPokemon(
    ...
    # print("sortedpokemontotal: ", len(sortedpokemontotal))
    ...
    for index, item in enumerate(sortedpokemontotal):
        # Only assign if deck id not already in modifiedpokemontotal
        for thing in modifiedpokemontotal:
            if item["deck"] == thing["deck"]:
                break
        else:
            if item["deck"] in allIds:
    ...
    for i, item in enumerate(results):
        ...
        for thing in modifiedpokemontotal:
            if thing["deck"] == item[0]:
                deckmon = thing["name"]
                already_assigned = True
                previouslevel = thing["level"]
                if thing["nickname"]:
        ...
        if item[0] not in everstonelist:
            ...
            try:  # temporary patch # TODO:nameが数字になるらしい
        ...
        # If name is None, set it to "Ditto" 🐞Shige
        if name is None:
            name = "Ditto"
            ...
            for thing in pokemontotal:
                if thing["deck"] == item[0]:
                    if name == "Eevee" or name == "Egg":
                        if nickname:
                            if any(p["name"] == thing["name"] and p["deck"] == thing["deck"] and 
                                   p["level"] == Level and p["nickname"] == nickname 
                                   for p in modifiedpokemontotal):
                                pass
                            else:
                            ...
                            if any(p["name"] == thing["name"] and p["deck"] == thing["deck"] and 
                                   p["level"] == Level for p in modifiedpokemontotal):
                                pass
                            else:
                                modifiedpokemontotal.append({
                                    "id": str(uuid.uuid4()),
                        ...
                        if nickname:
                            if any(p["name"] == name and p["deck"] == thing["deck"] and 
                                   p["level"] == Level and p["nickname"] == nickname 
                                   for p in modifiedpokemontotal):
                                pass
                            else:
    ...
    # print("Done")
    # print("modifiedpokemontotal:", len(modifiedpokemontotal))
...
def TagPokemon() -> Union[List[dict], None]:
    """
    ...
    # fixed duplication
    for item in reversed(tagmonlist):
        if item["deck"] in savedtags:
            for thing in modifiedtagmon:
                if item["deck"] == thing[1]:
                    break
            else:
                modifiedtagmon.append(item)
