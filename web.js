...
//     randomImg.src = images[Math.floor(Math.random() * images.length)]; // ﾗﾝﾀﾞﾑな画像のURLを設定
//     randomImg.style.position = 'absolute';
//     randomImg.style.top = (img.offsetTop - 100) + 'px'; // ｸﾘｯｸした画像の100px上に表示
//     randomImg.onload = function() {
//         randomImg.style.left = (img.offsetLeft + img.offsetWidth / 2 - randomImg.offsetWidth / 2) + 'px'; // ｸﾘｯｸした画像の左右中央に表示
//     }
//     img.parentNode.appendChild(randomImg); // img要素の親要素の子要素として追加
//     // 1.5秒後に画像を消す
//     setTimeout(() => {
...
//     // ﾗﾝﾀﾞﾑ画像を表示
//     let randomImg = document.createElement('img');
//     randomImg.src = images[Math.floor(Math.random() * images.length)]; // ﾗﾝﾀﾞﾑ画像のURLを設定
//     randomImg.style.position = 'absolute';
//     randomImg.style.top = img.offsetTop - 150 + 'px'; // ｸﾘｯｸ画像の30px上に表示
//     img.parentNode.insertBefore(randomImg, img);
// }
//     // ﾗﾝﾀﾞﾑな画像を表示
//     let randomImg = document.createElement('img');
//     randomImg.src = images[Math.floor(Math.random() * images.length)]; // ﾗﾝﾀﾞﾑな画像のURLを設定
//     randomImg.style.position = 'fixed';
//     randomImg.style.top = (img.getBoundingClientRect().top - 100) + 'px'; // ｸﾘｯｸした画像の100px上に表示
//     randomImg.style.left = img.getBoundingClientRect().left + 'px'; // ｸﾘｯｸした画像と同じ左位置に表示
//     document.body.appendChild(randomImg); // body要素の最後に追加
//     // 1.5秒後に画像を消す
//     setTimeout(() => {
//         randomImg.remove();
//     }, 1500);
